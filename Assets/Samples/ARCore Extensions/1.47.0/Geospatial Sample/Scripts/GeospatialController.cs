...
namespace Google.XR.ARCoreExtensions.Samples.Geospatial
{
    ...
    public class GeospatialController : MonoBehaviour
    {
        ...
        /// <summary>
        /// Material for the raycast marker
        /// </summary>
        public Material RaycastMarkerMaterial;
        ...
        private GameObject raycastMarker;
        ...
        // private void MoveWithCamera(Vector3 position)
        // {
        //     // Cast a ray from the screen touch or from a specific position in world space
        //     Ray ray = Camera.main.ScreenPointToRay(position); // Use position from your touch or desired location
        //
        ...
        //         if (hitInfo.collider != null)
        //         {
        //             hitInfo.transform.position = Camera.main.transform.position + initialLocalPosition;
        //         }
        //     }
        // }

        private void CreateMarkerOnVirtualObject(Vector3 position)
        {
            // Cast a ray from the screen touch or from a specific position in world space
            Ray ray = Camera.main.ScreenPointToRay(position); // Use position from your touch or desired location
            ...
            if (Physics.Raycast(ray, out var hitInfo))
            {
                ...
                {
                    // Process the hit
                    Debug.Log("Hit the virtual object: " + hitInfo.collider.gameObject.name);
        
                    raycastMarker = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                    raycastMarker.transform.position = hitInfo.point;
                    raycastMarker.transform.localScale = Vector3.one * 0.1f; // Adjust size of the sphere
                    raycastMarker.GetComponent<Renderer>().material = RaycastMarkerMaterial;
                    
                    // Optionally, you can do something with the virtual object, like applying a material change
                    hitInfo.collider.gameObject.GetComponent<Renderer>().material = RaycastMarkerMaterial;
                }
            }
        }

        private void UpdateMarkerPositionOnVirtualObject(Vector3 position)
        {
            // Cast a ray from the screen touch or from a specific position in world space
            Ray ray = Camera.main.ScreenPointToRay(position); // Use position from your touch or desired location
        ...
        private void PlaceAnchorByScreenTap(Vector2 position)
        {
            ...
            {
                // Get the first hit
                ARRaycastHit hit = planeHitResults[0];

                // Retrieve the ARPlane
                ARPlane detectedPlane = hit.trackable as ARPlane;
                Debug.Log("haha 1");

                if (detectedPlane != null)
                {
                    // Access the plane's MeshRenderer
                    MeshRenderer meshRenderer = detectedPlane.GetComponentInChildren<MeshRenderer>();
                    Debug.Log("haha 2");

                    if (meshRenderer != null)
                    {
                        // Save the original material color (optional, if reverting highlight later)
                        Color originalColor = meshRenderer.material.color;
                        Debug.Log($"originalColor: {originalColor}");

                        // Change the material color to highlight the plane
                        meshRenderer.material = RaycastMarkerMaterial;
                    }
                }
        ...
        private GeospatialAnchorHistory CreateHistory(Pose pose, AnchorType anchorType)
        {
            ...
            Debug.Log("Created anchor: " + Environment.NewLine + poseInfo);
